...
public class DashboardServiceImpl implements DashboardService {
	...
	// Getting list of indicators to show in panel
	@Override
	...
	// Getting list of indicators for organizational submissions
	...
	// Getting aggregated data for report
	...
	// Getting aggregated data in table format for dashboard
	@Override
...
//						.and("data.preferredDistrict").as("area")
	...
	// Getting block level aggregated data for selected district in dashboard
	@Override
	public Map<String, List<?>> getBlockTableData(Integer formId, String path, Integer districtId) {
		...
		if(formId == 1)
			match = Aggregation.match(Criteria.where("isVerified").is(true)
					.and("data.deactivated").ne(true)
					.and("formId").is(formId)
//					.and("data.areaLevel").is(2)
					.and(path.equals("total") ? "data.formId" : "data."+path).ne(null)
					.and("data.preferredBlock").in(blockList));
		else 
			match = Aggregation.match(Criteria.where("isVerified").is(true)
					.and("data.deactivated").ne(true)
					.and("formId").is(formId)
					.and("data.areaOfService").is(107)
					.and("data.preferredBlock").in(blockList)
					.and(dp.getPath().equals("preferredDistrict") ? "data.formId" : "data."+dp.getPath())
					.is(dp.getPath().equals("preferredDistrict")?2:dp.getTypeDetailId()));
		ProjectionOperation dataProject = Aggregation.project().and("data").as("data");
		
		ProjectionOperation project1 = null;
				
		if(path.equals("total") || path.equals("orgTotal"))
			...
			if(formId==1) {
				...
				project1 = Aggregation.project().and("data.preferredBlock").as("area")
						.and(when(where("data."+dp.getPath()).is(dp.getTypeDetailId())).then(1).otherwise(0)).as(path);
			}
		}
