...
public class TaskServiceImpl implements TaskService {
	...
	public void saveTask(Map<String, Object> data) {
		...
		try {
			...
			// Sending message to the volunteer
			if (reps != null)
	...
	// Gets all the tasks assigned to particular volunteer and returns the same
	@Override
	public Map<String, List<?>> getTaskList(String mobile) {
		SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");

		Map<String, List<?>> table = new HashMap<String, List<?>>();
		List<String> header = Arrays.asList("Activity Type#କାର୍ଯ୍ୟ ର ପ୍ରକାର", "Activity Details#କାର୍ଯ୍ୟ ର ବୀବରଣୀ ", "Coordinator Name#ସଂଯୋଜକଙ୍କ ନାମ ",
				"Coordinator Mobile No.#ସଂଯୋଜକଙ୍କ ମୋବାଇଲ୍ ନମ୍ବର", "Assigned Date#କାର୍ଯ୍ୟର ନିର୍ଦ୍ଦିଷ୍ଟ ତାରିଖ ସଂଲଗ୍ନ କରନ୍ତୁ");
...
//	Translates filter parameters into dynamic query and returns filtered list
	@Override
...
//		} 
		...
		if (isAssignedTask != null) {
			Criteria taskAssignmentCriteria = null;
			if(isAssignedTask == true)
				taskAssignmentCriteria = Criteria.where("isAssignedTask").is(true);
			else
				taskAssignmentCriteria = Criteria.where("isAssignedTask").ne(true);
...
//		} 
	...
	public ResponseEntity<String> generateCertificate(String mobile, OAuth2Authentication oauth) {
		...
		try {
			...
			String dir = configurableEnvironment.getProperty("certificate.path");
			File file = new File(dir);
			if (!file.exists())
				file.mkdirs();

			String name = "Volunteer" + "_" + new SimpleDateFormat("ddMMyyyyHHmmssSSSSSSS").format(new Date())
					+ ".pdf";
			outputPath = dir + "" + name;
...
//			writer.setPageEvent(headerFooter);
			...
			Paragraph p2;
			if(desig.get(0).getName().equals("District Level"))
				 p2 = new Paragraph(areaMap.get(userAreaId.get(0)), cfont);
			else
				p2= new Paragraph("", cfont);
